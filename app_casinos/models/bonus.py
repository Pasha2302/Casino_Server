...
class BonusAmount(models.Model):
    ...
    value = models.FloatField(verbose_name="(CAP) value", help_text=text_bonus_amount, null=True, blank=True,)
    ...
    # def clean(self):
        ...
        # if not self.unlimited and not all((self.value, self.symbol)):
        #     raise ValidationError('Values: (CAP) VALUE, SYMBOL are required, or set to UNLIMITED.')
...
class BonusValue(models.Model):
    ...
    value = models.FloatField(
        verbose_name="Value %", help_text=text_bonus_value_percentage, null=True, blank=True,
    )
    selected_source = models.CharField(max_length=20, choices=CHOICES_SOURCE, default='', )
    # def clean(self):
...
class BonusMinDep(models.Model):
    ...
    min_value = models.FloatField(
        verbose_name="Bonus min dep value", help_text=text_bonus_min_deposit, null=True, blank=True,
    )
    symbol = models.ForeignKey(
        BaseCurrency, related_name='bonus_min_dep', on_delete=models.SET_NULL, null=True, blank=True,
    )
    selected_source = models.CharField(max_length=20, choices=CHOICES_SOURCE, default='', )
...
class BonusMaxWin(models.Model):
    ...
    # def clean(self):
        # check_selected_source = self.selected_source if self.selected_source != 'undefined' else False
        # if not self.unlimited and not all((self.max_value, self.symbol)):
        #     raise ValidationError('Values: MAX VALUE, SYMBOL are required, or set to UNLIMITED.')
        # if not check_selected_source:
        #     raise ValidationError('Be sure to specify the data source (Selected source)')
...
class WageringBonusPlusDeposit(models.Model):
    ...
    bonus_plus_deposit = models.FloatField(
        verbose_name="(Wagering) Bonus + Deposit", help_text=text_bonus_wagering_bonus_deposit, null=True, blank=True,
    )
    bonus_only = models.FloatField(
        verbose_name="(Wagering) Bonus Only", help_text=text_bonus_wagering_bonus_only, null=True, blank=True,
    )
    selected_source = models.CharField(max_length=20, choices=CHOICES_SOURCE, default='', )
...
class SlotsWageringContribution(models.Model):
    ...
    value = models.FloatField(verbose_name="Wagering Contribution value %", null=True, blank=True)
...
class WageringContribution(models.Model):
    ...
    contribution_description = models.ForeignKey(
        ...
        on_delete=models.SET_NULL, null=True, blank=True, ) # help_text=text_bonus_wagering_contribution
    value = models.FloatField(verbose_name="Wagering Contribution value %", null=True, blank=True, )
    selected_source = models.CharField(max_length=20, choices=CHOICES_SOURCE, default='', )
...
class Bonus(models.Model):
    ...
    bonus_plus_freespins_value = models.FloatField(
    ...
    def save(self, *args, **kwargs):
        additionally = ''
        try:
            if self.casino and self.casino.name:
                additionally = self.casino.name
        except Exception as err:
            print(err)

        if not self.slug:
            self.slug = slugify(f"{self.name} {additionally}".strip())
            original_slug = self.slug
            counter = 1
            while Bonus.objects.filter(slug=self.slug).exists():
